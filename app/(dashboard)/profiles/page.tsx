...
const initialProfiles: Profile[] = [
  { profile_id: 1, name: "Default — Ornamental", min_moisture: 40, max_moisture: 80, watering_duration: 15 },
  { profile_id: 2, name: "Leafy Vegetables", min_moisture: 50, max_moisture: 90, watering_duration: 20 },
  { profile_id: 3, name: "Rose Nursery", min_moisture: 45, max_moisture: 75, watering_duration: 10 },
  { profile_id: 4, name: "Tropical — Orchids", min_moisture: 60, max_moisture: 95, watering_duration: 5 },
];

const initialSchedules: Schedule[] = [
  { schedule_id: 1, cron_expression: "AI_AUTO", active: true },
  { schedule_id: 2, cron_expression: "0 6,18 * * *", active: true },
  { schedule_id: 3, cron_expression: "0 7,12,17 * * *", active: true },
  { schedule_id: 4, cron_expression: "0 5 * * 1,3,5", active: false },
];

const initialZones: Zone[] = [
  { zone_id: 1, name: "Ornamental Garden", tag: "outdoor", profile_id: 1, schedule_id: 1, user_id: 1 },
  { zone_id: 2, name: "Lettuce Beds", tag: "greenhouse", profile_id: 2, schedule_id: 2, user_id: 1 },
  { zone_id: 3, name: "Rose Nursery", tag: "nursery", profile_id: 3, schedule_id: 1, user_id: 1 },
  { zone_id: 4, name: "Orchid House", tag: "greenhouse", profile_id: 4, schedule_id: 3, user_id: 1 },
];

const deviceCounts: Record<number, number> = { 1: 3, 2: 2, 3: 2, 4: 3 };
const sensorCounts: Record<number, number> = { 1: 5, 2: 4, 3: 3, 4: 6 };
...
export default function ConfigurationPage() {
  ...
  const openZoneModal = (zone?: Zone) => {
    // If editing, load the zone. If adding, set sensible defaults.
    setCurrentZone(zone || { name: "", tag: "", profile_id: profiles[0]?.profile_id || 1, schedule_id: schedules[0]?.schedule_id || 1 });
  ...
  const saveZone = () => {
    ...
    if (currentZone.zone_id) {
      // Update existing
      setZones(zones.map(z => z.zone_id === currentZone.zone_id ? { ...z, ...currentZone } as Zone : z));
    } else {
      // Create new
      const newId = Math.max(...zones.map(z => z.zone_id), 0) + 1;
      setZones([...zones, { ...currentZone, zone_id: newId, user_id: 1 } as Zone]);
    }
  ...
  const deleteZone = (id: number) => {
    if (window.confirm("Are you sure you want to delete this zone?")) {
      setZones(zones.filter(z => z.zone_id !== id));
    }
  };

  // --- PROFILES ---
  const openProfileModal = (profile?: Profile) => {
    setCurrentProfile(profile || { name: "", min_moisture: 40, max_moisture: 80, watering_duration: 15 });
  ...
  const saveProfile = () => {
    ...
    if (currentProfile.profile_id) {
      setProfiles(profiles.map(p => p.profile_id === currentProfile.profile_id ? { ...p, ...currentProfile } as Profile : p));
    } else {
      const newId = Math.max(...profiles.map(p => p.profile_id), 0) + 1;
      setProfiles([...profiles, { ...currentProfile, profile_id: newId } as Profile]);
    }
    setProfileModalOpen(false);
  };

  const deleteProfile = (id: number) => {
    if (zones.some(z => z.profile_id === id)) return alert("Cannot delete: Profile is assigned to a zone.");
  ...
  const openScheduleModal = (schedule?: Schedule) => {
    setCurrentSchedule(schedule || { cron_expression: "", active: true });
    setScheduleModalOpen(true);
  };

  const saveSchedule = () => {
    if (!currentSchedule.cron_expression) return alert("Cron expression is required!");
    if (currentSchedule.schedule_id) {
      setSchedules(schedules.map(s => s.schedule_id === currentSchedule.schedule_id ? { ...s, ...currentSchedule } as Schedule : s));
    } else {
      const newId = Math.max(...schedules.map(s => s.schedule_id), 0) + 1;
      setSchedules([...schedules, { ...currentSchedule, schedule_id: newId } as Schedule]);
    }
    setScheduleModalOpen(false);
  };

  const deleteSchedule = (id: number) => {
    if (zones.some(z => z.schedule_id === id)) return alert("Cannot delete: Schedule is assigned to a zone.");
    ...
    <div className="text-[#333] font-sans">
        ...
        <div>
          ...
          <div className="bg-white rounded-sm shadow-sm border border-[#e0e0e0] overflow-hidden">
            <table className="w-full border-collapse text-[13px]">
              ...
              <tbody>
                {zones.map((z) => (
                  <tr key={z.zone_id} className="hover:bg-[#f9fbe7] border-b border-[#f0f0f0]">
                    <td className="p-2.5 font-mono text-xs text-gray-400">{z.zone_id}</td>
                    <td className="p-2.5 font-medium">{z.name}</td>
                    <td className="p-2.5"><span className="inline-block px-2 py-0.5 rounded-sm text-[10px] font-bold bg-[#e3f2fd] text-[#1565c0]">{z.tag}</span></td>
                    <td className="p-2.5 text-xs">{profiles.find((p) => p.profile_id === z.profile_id)?.name || "—"}</td>
                    <td className="p-2.5 text-xs">{cronDesc(schedules.find((s) => s.schedule_id === z.schedule_id)?.cron_expression || "")}</td>
                    <td className="p-2.5 text-center">{deviceCounts[z.zone_id] || 0}</td>
                    <td className="p-2.5 text-center">{sensorCounts[z.zone_id] || 0}</td>
                    <td className="p-2.5">
                      <div className="flex gap-1">
                        ...
                        <button onClick={() => deleteZone(z.zone_id)} className="bg-[#c62828] text-white p-1 rounded-sm hover:brightness-110"><Trash2 className="w-3 h-3" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      {activeTab === "profiles" && (
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {profiles.map((p) => {
              const usedBy = zones.filter((z) => z.profile_id === p.profile_id).map((z) => z.name);
              return (
                <div key={p.profile_id} className="bg-white rounded-sm shadow-sm border border-[#e0e0e0] overflow-hidden">
                  ...
                  <div className="p-4 space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                      <div><div className="text-[10px] text-[#999] uppercase font-bold tracking-wide mb-0.5">Moisture</div><div className="text-[13px] font-medium">{p.min_moisture}% — {p.max_moisture}%</div></div>
                      <div><div className="text-[10px] text-[#999] uppercase font-bold tracking-wide mb-0.5">Watering Duration</div><div className="text-[13px] font-medium">{p.watering_duration} min</div></div>
                    </div>
                    ...
                    <div className="flex gap-2 pt-2 border-t border-gray-100">
                      ...
                      <button onClick={() => deleteProfile(p.profile_id)} className="bg-[#c62828] text-white px-2 py-1 text-[10px] font-bold uppercase rounded-sm inline-flex items-center gap-1 hover:brightness-110"><Trash2 className="w-3 h-3" /> Delete</button>
                    </div>
                  </div>
                </div>
              );
        ...
        <div>
          ...
          <div className="bg-white rounded-sm shadow-sm border border-[#e0e0e0] overflow-hidden">
            <table className="w-full border-collapse text-[13px]">
              ...
              <tbody>
                {schedules.map((s) => {
                  const usedBy = zones.filter((z) => z.schedule_id === s.schedule_id).map((z) => z.name);
                  return (
                    <tr key={s.schedule_id} className="hover:bg-[#f9fbe7] border-b border-[#f0f0f0]">
                      <td className="p-2.5 font-mono text-xs text-gray-400">{s.schedule_id}</td>
                      <td className="p-2.5 font-mono text-sm">{s.cron_expression}</td>
                      <td className="p-2.5 text-xs text-gray-500">{cronDesc(s.cron_expression)}</td>
                      <td className="p-2.5">
                        <span className={`inline-block px-2 py-0.5 rounded-sm text-[10px] font-bold ${s.active ? 'bg-[#e0f2f1] text-[#00695c]' : 'bg-[#fff3e0] text-[#e65100]'}`}>{s.active ? 'Active' : 'Inactive'}</span>
                      </td>
                      ...
                      <td className="p-2.5">
                        <div className="flex gap-1">
                          ...
                          <button onClick={() => deleteSchedule(s.schedule_id)} className="bg-[#c62828] text-white p-1 rounded-sm hover:brightness-110"><Trash2 className="w-3 h-3" /></button>
                        </div>
                      </td>
                    </tr>
                  );
        ...
        <div className="fixed inset-0 bg-black/45 flex items-center justify-center z-1000">
          <div className="bg-white rounded-md w-130 max-w-[95vw] shadow-2xl">
            <div className="p-4 border-b border-[#eee] flex justify-between items-center text-[15px] font-medium">
              <span>{currentZone.zone_id ? "Edit Zone" : "Add Zone"}</span>
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                <label className="block text-[11px] font-bold uppercase tracking-wide text-[#666] mb-1">Tag</label>
                <input value={currentZone.tag || ""} onChange={(e) => setCurrentZone({ ...currentZone, tag: e.target.value })} type="text" placeholder="e.g. outdoor, greenhouse" className="w-full border border-[#ddd] p-2 text-[13px] rounded-sm outline-none focus:border-[#00695c]" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] font-bold uppercase tracking-wide text-[#666] mb-1">Assign Profile</label>
                  <select value={currentZone.profile_id} onChange={(e) => setCurrentZone({ ...currentZone, profile_id: Number(e.target.value) })} className="w-full border border-[#ddd] p-2 text-[13px] rounded-sm outline-none focus:border-[#00695c] bg-white">
                ...
                <div>
                  ...
                  <select value={currentZone.schedule_id} onChange={(e) => setCurrentZone({ ...currentZone, schedule_id: Number(e.target.value) })} className="w-full border border-[#ddd] p-2 text-[13px] rounded-sm outline-none focus:border-[#00695c] bg-white">
                    {schedules.map(s => <option key={s.schedule_id} value={s.schedule_id}>{s.cron_expression}</option>)}
                  </select>
                </div>
              </div>
            </div>
      ...
      {isProfileModalOpen && (
        <div className="fixed inset-0 bg-black/45 flex items-center justify-center z-1000">
          <div className="bg-white rounded-md w-130 max-w-[95vw] shadow-2xl">
            <div className="p-4 border-b border-[#eee] flex justify-between items-center text-[15px] font-medium">
              <span>{currentProfile.profile_id ? "Edit Profile" : "Add Profile"}</span>
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input value={currentProfile.min_moisture || 0} onChange={(e) => setCurrentProfile({ ...currentProfile, min_moisture: Number(e.target.value) })} type="number" className="w-full border border-[#ddd] p-2 text-[13px] rounded-sm outline-none focus:border-[#00695c]" />
                </div>
                <div>
                  <label className="block text-[11px] font-bold uppercase tracking-wide text-[#666] mb-1">Max Moisture (%)</label>
                  <input value={currentProfile.max_moisture || 0} onChange={(e) => setCurrentProfile({ ...currentProfile, max_moisture: Number(e.target.value) })} type="number" className="w-full border border-[#ddd] p-2 text-[13px] rounded-sm outline-none focus:border-[#00695c]" />
                </div>
              </div>
              <div>
                <label className="block text-[11px] font-bold uppercase tracking-wide text-[#666] mb-1">Watering Duration (minutes)</label>
                <input value={currentProfile.watering_duration || 0} onChange={(e) => setCurrentProfile({ ...currentProfile, watering_duration: Number(e.target.value) })} type="number" className="w-full border border-[#ddd] p-2 text-[13px] rounded-sm outline-none focus:border-[#00695c]" />
              </div>
            </div>
            <div className="p-3 border-t border-[#eee] bg-[#fafafa] flex justify-end gap-2">
      ...
      {isScheduleModalOpen && (
        <div className="fixed inset-0 bg-black/45 flex items-center justify-center z-1000">
          <div className="bg-white rounded-md w-130 max-w-[95vw] shadow-2xl">
            <div className="p-4 border-b border-[#eee] flex justify-between items-center text-[15px] font-medium">
              <span>{currentSchedule.schedule_id ? "Edit Schedule" : "Add Schedule"}</span>
            ...
            <div className="p-5 space-y-4">
              <div>
                ...
                <input value={currentSchedule.cron_expression || ""} onChange={(e) => setCurrentSchedule({ ...currentSchedule, cron_expression: e.target.value })} type="text" placeholder="e.g. 0 6,18 * * *" className="w-full border border-[#ddd] p-2 text-[13px] rounded-sm outline-none focus:border-[#00695c]" />
                <p className="text-[10px] text-gray-400 mt-1">Format: minute hour day-of-month month day-of-week</p>
              </div>
              <div className="flex items-center gap-2 mt-4">
                <input id="active-checkbox" type="checkbox" checked={currentSchedule.active} onChange={(e) => setCurrentSchedule({ ...currentSchedule, active: e.target.checked })} className="w-4 h-4 accent-[#00695c]" />
                <label htmlFor="active-checkbox" className="text-[13px] font-medium text-[#333] cursor-pointer">Set as Active</label>
              </div>
            </div>
            <div className="p-3 border-t border-[#eee] bg-[#fafafa] flex justify-end gap-2">
