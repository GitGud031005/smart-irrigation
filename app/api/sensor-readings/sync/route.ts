...
export async function POST() {
  try {
    ...
    const lastStored = await getLatestSensorReading().then(
      (r) => r?.recordedAt,
    );
    ...
    const [soilData, tempData, humData] = await Promise.all([
      getFeedData(
        process.env.ADAFRUIT_IO_FEED_SOIL_MOISTURE || "soil-moisture",
        startTime ? { start_time: startTime } : undefined,
      ),
      getFeedData(
        process.env.ADAFRUIT_IO_FEED_TEMPERATURE || "temperature",
        startTime ? { start_time: startTime } : undefined,
      ),
      getFeedData(
        process.env.ADAFRUIT_IO_FEED_HUMIDITY || "humidity",
        startTime ? { start_time: startTime } : undefined,
      ),
    ]);
    ...
    const tagged: TaggedDatum[] = [
      ...(soilData as AdafruitDatum[]).map((d) => ({
        metric: "soilMoisture" as MetricKey,
        value: parseFloat(d.value),
        ts: new Date(d.created_at),
      })),
      ...(tempData as AdafruitDatum[]).map((d) => ({
        metric: "temperature" as MetricKey,
        value: parseFloat(d.value),
        ts: new Date(d.created_at),
      })),
      ...(humData as AdafruitDatum[]).map((d) => ({
        metric: "humidity" as MetricKey,
        value: parseFloat(d.value),
        ts: new Date(d.created_at),
      })),
    ]
    ...
    if (tagged.length === 0) {
      return NextResponse.json({ message: "No new data from any feed." });
    }
    ...
    const inserts = await createSensorReadingsBatch(toInsert);
    return NextResponse.json({
      message: `Inserted ${inserts} new sensor readings.`,
    });
  } catch (error) {
